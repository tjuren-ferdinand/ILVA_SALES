...
export function Showcase() {
  ...
  const selectedDemo = DEMO_PRODUCTS.find((p) => p.id === selectedId) ?? DEMO_PRODUCTS[0]
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setFetchError(null)
    fetch(`/api/ilva/product?url=${encodeURIComponent(urlParam)}`)
      .then((res) => res.json())
      .then((data) => {
        if (cancelled) return
      ...
      .catch((err) => {
        if (cancelled) return
        setFetchError(err instanceof Error ? err.message : 'Kunde inte hämta produkten')
      })
      .finally(() => {
  ...
  const shown = product ?? selectedDemo
  ...
  const shareText = useMemo(() => {
    return `Titta på ILVA ${shown.name} i ${activeColor.name.toLowerCase()} ${material.name.toLowerCase()} — ${formatPrice(shown.ordinaryPrice)}`
  }, [shown, activeColor, material])
      ...
      <div className="mx-auto w-full max-w-3xl overflow-hidden rounded-[2.5rem] border border-white/50 bg-white/80 shadow-2xl backdrop-blur-2xl">
        ...
        <div className="p-6 md:p-10">
          ...
          <div className="mx-auto mt-8 max-w-lg">
            ...
            {urlParam && loading && (
              <p className="mb-4 text-sm text-muted">Hämtar produkt från ILVA...</p>
            )}
            {urlParam && fetchError && (
              <div className="mb-4 rounded-2xl border border-amber-200/50 bg-amber-50/60 p-4 text-sm text-amber-800">
                {fetchError}
              </div>
            )}
      ...
      <div className="surface p-6">
        <div className="flex items-start gap-3">
          ...
          <p className="text-sm text-muted">
            Produktvyn visar riktiga ILVA-produktbilder, priser och artikelnummer. Färg- och materialalternativen är visuella exempel.
          </p>
        </div>
      </div>
    </div>
